"""Action-bound, expiring, single-use approvals shared by guard harnesses.
...
class ApprovalStore:
    ...
    def find_pending(
        self, *, digest: str, requester: str, harness: str | None = None,
    ) -> str | None:
        """Return the newest live pending request for this exact action.

        Harness adapters may evaluate the same blocked action more than once
        while an operator is deciding. Reusing the authenticated request keeps
        the dashboard from accumulating duplicate approvals without weakening
        the digest/requester binding.
        """
        if len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
            return None
        now = self._now()
