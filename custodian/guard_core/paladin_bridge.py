"""Paladin integration shared by every Custodian guard adapter.
...
``custodian/`` may *import* ``paladin``; the kernel and its adapters stay
brand-neutral. So this bridge reaches Paladin exactly the way a guard
reaches ``git`` or a coding agent's CLI: as an **external tool** (the
...
CLI is absent, no vault is configured, or the vault is locked. A guard
adapter is fully functional without it -- credential actions simply escalate
to a human instead of resolving from a vault.

Two real mechanisms connect a guard to Paladin:

1. **Git credential helper** -- transparent secret delivery. ``paladin
   git-setup <host> <ref>`` wires git so any ``git push`` / ``git fetch`` the
   agent runs resolves the token from the encrypted vault at the moment git
   asks. The token never lands in git config, a remote URL, a command line,
   or the agent's context. This is the "the agent doesn't know the password,
   so it checks Paladin first" path, fully transparent to the model.
...
helper, or an explicit ``paladin exec``), never inside a pre-action hook that
fires on every tool call.

Moved here (from ``custodian.codex_guard.paladin_bridge``) so every guard
adapter (Codex, Claude, OpenCode, Hermes) shares one implementation instead of
each depending on codex-guard's package for it -- see
``tests/test_architecture_boundaries.py::test_no_guard_imports_another_guard``.
``custodian.codex_guard.paladin_bridge`` re-exports these same names for
backward compatibility with already-published callers.
"""
