...
* ``user:cli``             — the human at the Paladin CLI
...
from paladin.errors import GrantDeniedError, PaladinError
...
def band_index(band: str) -> int:
    try:
        ...
        raise PaladinError(f"unknown band {band!r} (expected one of {BAND_ORDER})")
